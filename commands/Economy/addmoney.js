...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        .setFields({ 
            name: "Использование",
            value: `\`${await db.get(message, "guild", "settings", "prefix")}setmoney <money | bank> <Кол-во> [@User | User-id]\`
...
let mentioned;
       ...
       const bank = await db.get(message, "user", "economy", "bank", mentioned);
        ...
        .setAuthor({ name: `${username}`, iconURL: avatar })
            .setDescription(`
Добавлено ${args[1]} | ${currency} пользователю <@${mentioned}>`)
